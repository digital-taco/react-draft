...
function getPackageName() {
  ...
  let { name } = fs.existsSync(packagePath) ? require(packagePath) : null
  return name
}
...
function getHMRPath() {
  ...
  let hotMiddlewareScriptPath = path.resolve(__dirname, hotMiddlewareScript)
  return fs.existsSync(hotMiddlewareScriptPath) ? hotMiddlewareScriptPath : path.resolve('.', hotMiddlewareScript)
}
