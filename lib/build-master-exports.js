...
if (exports.${componentHash}) exports.${componentHash}.meta = ${JSON.stringify(componentMeta)}\n`
      ...
      componentMetaStr += `exports.${componentHash} = ${JSON.stringify(componentMeta)}\n`
...
exports.EmptyDemo.meta = {componentHash: 'EmptyDemo'}\n
  ...
  const masterExportsFile = `${importList +
    metaList}\n${componentTreeExport}\nexport default exports`

  const componentMetaFile = `
const exports = {}
${componentMetaStr}
export default exports
`

  const componentTreeFile = `
export default ${JSON.stringify(componentTree, null, 2)}
`
  // Write it to the master exports file
  log.debug('Building master exports file')
  if (!fs.existsSync(path.resolve(__dirname, '../out'))) {
    fs.mkdirSync(path.resolve(__dirname, '../out'))
  }

  fs.writeFileSync(path.resolve(__dirname, '../out', 'component-list.js'), masterExportsFile)
  log.debug('Component list file built', 'server')

  fs.writeFileSync(path.resolve(__dirname, '../out', 'component-meta.js'), componentMetaFile)
  log.debug('Component meta file built', 'server')

  fs.writeFileSync(path.resolve(__dirname, '../out', 'component-tree.js'), componentTreeFile)
  log.debug('Component tree file built', 'server')
